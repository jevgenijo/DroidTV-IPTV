...
try:											# Let's see if cookielib is available
	...
	xbmc.log('[OZO.TV] cookielib is not available..')
	pass
else:
...
try:
	...
	xbmc.log('[OZO.TV] module json is not available. using demjson')
	import demjson
...
class rodnoe:
	...
	def _request(self, cmd, params, inauth = None):
		...
		xbmc.log('[OZO.TV] Requesting %s' % url);
		...
		xbmc.log('[OZO.TV] Got %s' % rez, level=xbmc.LOGDEBUG)
		
		
		try:
			res = JSONDECODE(rez)
		except:
			xbmc.log('[OZO.TV] Error.. :(')
			
		#xbmc.log('[OZO.TV] Got JSON: %s' % res)
		
		self._errors_check(res)
		
		if COOKIEJAR != None:
			xbmc.log('[OZO.TV] Saving cookies: %s' % COOKIEFILE)
	...
	def initTimeFix(self):
		...
		if self.auto_timezone:
			xbmc.log('[OZO.TV] detecting local timezone')
			if time.timezone:
				self.time_zone = time.timezone / -60
			else:
				self.time_zone = 0
		xbmc.log('[OZO.TV] timezone set to %s' % self.time_zone)
		(val, opt) = self.getSettingCurrent('time_shift')
		self.timeshift = int(val)
		xbmc.log('[OZO.TV] time shift set to %s' % self.timeshift)
	...
	def _errors_check(self, json):
		if 'error' in json:
			...
			if 'message' in err:
				...
				if message == None:
					message = err['code']
				xbmc.log('[OZO.TV] ERROR: %s' % message.encode('utf8'))
				self.AUTH_OK = False
	
	def getLast(self):
		if self.last_list and 'ttl' in self.last_list:
			if int(self.last_list['ttl']) < time():
				xbmc.log('[OZO.TV] Last list expired')
				self.last_list = None
		if not self.last_list:
			...
			try:
				...
				xbmc.log('[OZO.TV] Error loading last list %s' % e)
				res = self.getChannelsList()
			else:
				xbmc.log('[OZO.TV] last list loaded')
			f.close()
	...
	def getChannelsList(self):
		...
		try:
			...
			xbmc.log('[OZO.TV] Error saving last list %s' % e)
		else:
			xbmc.log('[OZO.TV] last list stored')
		f.close()
	...
	def getSettingsList(self):
		...
		xbmc.log('[OZO.TV] settings: %s' % res)
		return res
